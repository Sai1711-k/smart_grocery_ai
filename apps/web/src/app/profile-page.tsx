...
export function ProfilePage({ onNavigate }: { onNavigate?: (view: string) => void }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 pb-24 transition-colors">
      ...
      <div className="px-5 -mt-12 relative z-10 space-y-4">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-neutral-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            <h2 className="text-base font-bold text-neutral-800">Personal Info</h2>
            {!isEditing ? (
              <button
        ...
        {/* ── Quick Links ── */}
        <div className="bg-white rounded-3xl shadow-sm border border-neutral-100 p-2">
          {isAdmin && (
            <ProfileLink
          ...
          />
          <div className="h-px bg-neutral-100 my-1 mx-3" />
          <ProfileLink
            icon={<MapPin size={18} />}
  ...
  return (
    <div>
      ...
      <input
        ...
        disabled={disabled}
        className={`w-full px-4 py-3 rounded-xl border text-sm font-medium transition-colors outline-none ${disabled ? 'bg-neutral-50 border-neutral-100 text-neutral-400 cursor-not-allowed' : 'bg-white border-neutral-200 text-neutral-800 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100'}`}
      />
    </div>
  );
}
...
function ProfileLink({ icon, label, badge, badgeColor, iconBg, onClick }: { icon: React.ReactNode; label: string; badge?: string; badgeColor?: string; iconBg?: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="w-full flex items-center justify-between p-3.5 hover:bg-neutral-50 rounded-2xl transition-colors">
      <div className="flex items-center gap-3.5 text-neutral-700 font-semibold text-sm">
