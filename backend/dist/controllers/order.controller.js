...
const mockOrders = []; // Store mock orders in memory for presentation mode
class OrderController {
    static async checkout(req, res) {
        try {
            const userId = req.user.id;
            const { cartItems, // array of { product_id, provider_id, quantity, price, product_name, product_image }
            totalAmount, subtotal, tax, deliveryFee, deliveryAddress, customerName, customerEmail, paymentMethod } = req.body;
            ...
            for (const item of cartItems) {
                if (item.provider_id && item.product_id) {
                    const { error: rpcErr } = await supabase_1.supabaseAdmin.rpc('decrement_stock', {
            ...
            const { cartItems = [], totalAmount = 0, subtotal = 0, tax = 0, deliveryFee = 0, deliveryAddress = '', customerName = 'Guest', paymentMethod = 'Online' } = req.body;
            const mockOrder = {
