...
export async function GET() {
  try {
    ...
    // Check if snacks are present; if not, append fallbackSnacks
    const hasSnacks = formatted.some((p: any) => p.category === 'Snacks');
    if (!hasSnacks) {
      const snackFormatted = fallbackSnacks.map((s, idx) => ({
        id: `snack-item-${idx}`,
        provider_id: 'freshcart-store',
        name: s.name,
        description: `Fresh, crispy and delicious ${s.name}.`,
        category: s.category,
        price: s.price,
        unit: 'pack',
      ...
      formatted = [...formatted, ...snackFormatted];
    }
